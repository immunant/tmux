...
unsafe extern "C" fn cmd_select_pane_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if (*self_0).entry == &cmd_last_pane_entry as *const cmd_entry ||
           0 != args_has(args, 108 as u_char) {
        lastwp = (*w).last;
        if lastwp == 0 as *mut libc::c_void as *mut window_pane {
            cmdq_error(item,
                       b"no last pane\x00" as *const u8 as
                           *const libc::c_char);
            return CMD_RETURN_ERROR
        } else {
            if 0 != args_has((*self_0).args, 101 as u_char) {
                (*lastwp).flags &= !64i32
            } else if 0 != args_has((*self_0).args, 100 as u_char) {
                (*lastwp).flags |= 64i32
            } else {
                server_unzoom_window(w);
                window_redraw_active_switch(w, lastwp);
                if 0 != window_set_active_pane(w, lastwp) {
    ...
    } else if 0 != args_has(args, 109 as u_char) ||
                  0 != args_has(args, 77 as u_char) {
        if 0 != args_has(args, 109 as u_char) && 0 == window_pane_visible(wp)
           {
            return CMD_RETURN_NORMAL
        } else {
            lastwp = marked_pane.wp;
            if 0 != args_has(args, 77 as u_char) ||
                   0 != server_is_marked(s, wl, wp) {
                server_clear_marked();
            ...
            if markedwp != 0 as *mut libc::c_void as *mut window_pane {
                server_redraw_window_borders((*markedwp).window);
                server_status_window((*markedwp).window);
            }
            return CMD_RETURN_NORMAL
        }
    } else if 0 != args_has((*self_0).args, 80 as u_char) ||
                  0 != args_has((*self_0).args, 103 as u_char) {
        if 0 != args_has(args, 80 as u_char) {
            style = args_get(args, 80 as u_char);
            if style_parse(&grid_default_cell as *const grid_cell,
                           &mut (*wp).colgc as *mut grid_cell, style) ==
                   1i32.wrapping_neg() {
                cmdq_error(item,
        ...
        if 0 != args_has((*self_0).args, 103 as u_char) {
            cmdq_print(item, b"%s\x00" as *const u8 as *const libc::c_char,
                       style_tostring(&mut (*wp).colgc as *mut grid_cell));
        }
        return CMD_RETURN_NORMAL
    } else {
        ...
        if wp == 0 as *mut libc::c_void as *mut window_pane {
            return CMD_RETURN_NORMAL
        } else if 0 != args_has((*self_0).args, 101 as u_char) {
            (*wp).flags &= !64i32;
            return CMD_RETURN_NORMAL
        } else if 0 != args_has((*self_0).args, 100 as u_char) {
            (*wp).flags |= 64i32;
            ...
            if 0 != args_has((*self_0).args, 84 as u_char) {
                pane_title =
                    format_single(item,
                                  args_get((*self_0).args, 84 as u_char), c,
                                  s, wl, wp);
                screen_set_title(&mut (*wp).base as *mut screen, pane_title);
            ...
            if wp == (*w).active {
                ...
                if 0 == window_pane_visible(wp) {
                    cmdq_error(item,
                               b"pane not visible\x00" as *const u8 as
                                   *const libc::c_char);
                    return CMD_RETURN_ERROR
                } else {
                    window_redraw_active_switch(w, wp);
                    if 0 != window_set_active_pane(w, wp) {
...
pub static mut cmd_last_pane_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"last-pane\x00" as *const u8 as *const libc::c_char,
                  alias: b"lastp\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_36{template:
                                     b"det:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-de] [-t target-window]\x00" as *const u8 as
                          *const libc::c_char,
                  source:
...
pub static mut cmd_select_pane_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"select-pane\x00" as *const u8 as *const libc::c_char,
                  alias: b"selectp\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_36{template:
                                     b"DdegLlMmP:RT:t:U\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-DdegLlMmRU] [-P style] [-T title] [-t target-pane]\x00"
