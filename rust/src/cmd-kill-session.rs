    ...
    if 0 != args_has(args, 67 as u_char) {
        ...
        sloop =
            sessions_RB_MINMAX(&mut sessions as *mut sessions,
                               1i32.wrapping_neg());
        while sloop != 0 as *mut libc::c_void as *mut session &&
                  { stmp = sessions_RB_NEXT(sloop); 0 != 1i32 } {
            if sloop != s {
...
pub static mut cmd_kill_session_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"kill-session\x00" as *const u8 as *const libc::c_char,
                  alias: 0 as *const libc::c_char,
                  args:
                      unnamed_27{template:
                                     b"aCt:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
