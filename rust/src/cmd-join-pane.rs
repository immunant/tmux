...
pub const TTY_VT102: unnamed_1 = 2;
#[derive ( Copy , Clone )]
#[repr ( C )]
pub struct job {
...
pub const TTY_VT320: unnamed_1 = 4;
...
pub const TTY_VT420: unnamed_1 = 5;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const TTY_VT220: unnamed_1 = 3;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const TTY_VT101: unnamed_1 = 1;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const TTY_UNKNOWN: unnamed_1 = 6;
...
pub const TTY_VT100: unnamed_1 = 0;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
unsafe extern "C" fn cmd_join_pane_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if (*self_0).entry == &cmd_join_pane_entry as *const cmd_entry {
        not_same_window = 1i32
    } else { not_same_window = 0i32 }
    dst_s = (*item).target.s;
    ...
    if 0 != not_same_window && src_w == dst_w {
        cmdq_error(item,
                   b"can\'t join a pane to its own window\x00" as *const u8 as
                       *const libc::c_char);
        return CMD_RETURN_ERROR
    } else if 0 == not_same_window && src_wp == dst_wp {
        cmdq_error(item,
        ...
        lc =
            layout_split_pane(dst_wp, type_0, size,
                              args_has(args, 98 as u_char), 0i32);
        if lc == 0 as *mut libc::c_void as *mut layout_cell {
            cmdq_error(item,
                       b"create pane failed: pane too small\x00" as *const u8
            ...
            loop  {
                ...
                if (*src_wp).entry.tqe_next !=
                       0 as *mut libc::c_void as *mut window_pane {
                    (*(*src_wp).entry.tqe_next).entry.tqe_prev =
                        &mut (*src_wp).entry.tqe_next as *mut *mut window_pane
                } else {
                    let ref mut fresh1 =
                        (*(&mut (*dst_w).panes as
                               *mut window_panes)).tqh_last;
            ...
            if window_count_panes(src_w) == 0i32 as libc::c_uint {
...
pub static mut cmd_join_pane_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"join-pane\x00" as *const u8 as *const libc::c_char,
                  alias: b"joinp\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_30{template:
                                     b"bdhvp:l:s:t:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-bdhv] [-p percentage|-l size] [-s src-pane] [-t dst-pane]\x00"
...
pub static mut cmd_move_pane_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"move-pane\x00" as *const u8 as *const libc::c_char,
