 ...
 -> cmd_retval {
    ...
    if 0 != args_has((*self_0).args, 68 as u_char) {
        ...
        while wp != 0 as *mut libc::c_void as *mut window_pane {
            ...
            if wp2 == 0 as *mut libc::c_void as *mut window_pane { break ; }
        ...
        loop  {
            ...
            *fresh5 = wp;
            let ref mut fresh6 =
                (*(&mut (*w).panes as *mut window_panes)).tqh_last;
            *fresh6 = &mut (*wp).entry.tqe_next as *mut *mut window_pane;
            if !(0 != 0i32) { break ; }
        }
        lc = (*wp).layout_cell;
        xoff = (*wp).xoff;
...
pub static mut cmd_rotate_window_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                  ...
                  args:
                      unnamed_38{template:
                                     b"Dt:U\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-DU] [-t target-window]\x00" as *const u8 as
                          *const libc::c_char,
                  source:
