...
unsafe extern "C" fn cmd_detach_client_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    let mut cmd: *const libc::c_char = args_get(args, 69 as u_char);
    c = cmd_find_client(item, args_get(args, 116 as u_char), 0i32);
    if c == 0 as *mut libc::c_void as *mut client {
        return CMD_RETURN_ERROR
    } else if (*self_0).entry == &cmd_suspend_client_entry as *const cmd_entry
     {
        ...
        if 0 != args_has(args, 115 as u_char) {
            ...
            if s == 0 as *mut libc::c_void as *mut session {
                ...
                cloop = (*(&mut clients as *mut clients)).tqh_first;
                while cloop != 0 as *mut libc::c_void as *mut client {
                    if (*cloop).session == s {
                        if cmd !=
                               0 as *mut libc::c_void as *const libc::c_char {
                            server_client_exec(cloop, cmd);
                        } else { server_client_detach(cloop, msgtype); }
                    }
                    cloop = (*cloop).entry.tqe_next
                }
                return CMD_RETURN_STOP
            }
        } else if 0 != args_has(args, 97 as u_char) {
            cloop = (*(&mut clients as *mut clients)).tqh_first;
            while cloop != 0 as *mut libc::c_void as *mut client {
                if (*cloop).session != 0 as *mut libc::c_void as *mut session
                       && cloop != c {
            ...
            } else { server_client_detach(c, msgtype); }
            return CMD_RETURN_STOP
        }
    };
}
