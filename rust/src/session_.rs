...
pub static mut sessions: sessions =
    unsafe { sessions{rbh_root: 0 as *const session as *mut session,} };
#[no_mangle]
pub static mut session_groups: session_groups =
    unsafe {
        session_groups{rbh_root:
                           0 as *const session_group as *mut session_group,}
    };
#[no_mangle]
    ...
    if (*elm).entry.rbe_left == 0 as *mut libc::c_void as *mut session {
    ...
    } else if (*elm).entry.rbe_right == 0 as *mut libc::c_void as *mut session
     {
        ...
        if !parent.is_null() {
            ...
            current_block = 11791496542913101533;
        } else { current_block = 11791496542913101533; }
    }
    match current_block {
        9386390421034826751 => {
            parent = (*elm).entry.rbe_parent;
 ...
 -> *mut session_group {
    ...
    if (*elm).entry.rbe_left == 0 as *mut libc::c_void as *mut session_group {
        ...
        if !parent.is_null() {
            ...
            current_block = 720964374014813935;
        } else { current_block = 720964374014813935; }
    }
    match current_block {
        9386390421034826751 => {
            parent = (*elm).entry.rbe_parent;
        ...
        session{id: 0,
                ...
                lock_timer:
                    event{ev_active_next:
                              unnamed_14{tqe_next: 0 as *mut event,
                                         tqe_prev: 0 as *mut *mut event,},
                          ev_next:
                              unnamed_37{tqe_next: 0 as *mut event,
                                         tqe_prev: 0 as *mut *mut event,},
                          ev_timeout_pos:
                              unnamed_8{ev_next_with_common_timeout:
                                            unnamed_20{tqe_next:
                                                           0 as *mut event,
                                                       tqe_prev:
                                                           0 as
                ...
                references: 0,
                gentry:
                    unnamed_15{tqe_next: 0 as *mut session,
                               tqe_prev: 0 as *mut *mut session,},
                entry:
                    unnamed_23{rbe_left: 0 as *mut session,
            ...
            if (*s).gentry.tqe_next != 0 as *mut libc::c_void as *mut session
               {
                ...
                let ref mut fresh5 =
                    (*(&mut (*sg).sessions as *mut unnamed_2)).tqh_last;
                *fresh5 = (*s).gentry.tqe_prev
            }
            *(*s).gentry.tqe_prev = (*s).gentry.tqe_next;
            if !(0 != 0i32) { break ; }
        }
        if (*(&mut (*sg).sessions as *mut unnamed_2)).tqh_first ==
               0 as *mut libc::c_void as *mut session {
...
pub unsafe extern "C" fn session_group_contains(mut target: *mut session)
 -> *mut session_group {
    ...
    loop  {
        if sg != 0 as *mut libc::c_void as *mut session_group {
            s = (*(&mut (*sg).sessions as *mut unnamed_2)).tqh_first;
            loop  {
                if s != 0 as *mut libc::c_void as *mut session {
                    if s == target {
                        return sg
...
unsafe extern "C" fn session_group_synchronize1(mut target: *mut session,
    ...
    if (*ww).rbh_root == 0 as *mut libc::c_void as *mut winlink {
        ...
        while !((*(&mut old_windows as *mut winlinks)).rbh_root ==
                    0 as *mut libc::c_void as *mut winlink) {
...
pub unsafe extern "C" fn session_has(mut s: *mut session, mut w: *mut window)
 -> libc::c_int {
    let mut wl: *mut winlink = 0 as *mut winlink;
    wl = (*(&mut (*w).winlinks as *mut unnamed_33)).tqh_first;
    loop  {
        if wl != 0 as *mut libc::c_void as *mut winlink {
            if (*wl).session == s {
                return 1i32
 ...
 -> *mut session_group {
    let mut sg: session_group =
        session_group{name: 0 as *const libc::c_char,
                      sessions:
                          unnamed_2{tqh_first: 0 as *mut session,
                                    tqh_last: 0 as *mut *mut session,},
                      entry:
                          unnamed_17{rbe_left: 0 as *mut session_group,
...
pub unsafe extern "C" fn session_group_new(mut name: *const libc::c_char)
 -> *mut session_group {
    ...
    if sg != 0 as *mut libc::c_void as *mut session_group {
        ...
        loop  {
            let ref mut fresh12 =
                (*(&mut (*sg).sessions as *mut unnamed_2)).tqh_first;
            *fresh12 = 0 as *mut session;
            let ref mut fresh13 =
                (*(&mut (*sg).sessions as *mut unnamed_2)).tqh_last;
            *fresh13 =
                &mut (*(&mut (*sg).sessions as *mut unnamed_2)).tqh_first as
                    *mut *mut session;
            if !(0 != 0i32) { break ; }
        }
...
pub unsafe extern "C" fn session_group_synchronize_to(mut s: *mut session)
 -> () {
    ...
    if sg == 0 as *mut libc::c_void as *mut session_group {
        ...
        target = (*(&mut (*sg).sessions as *mut unnamed_2)).tqh_first;
