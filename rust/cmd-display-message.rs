...
unsafe extern "C" fn cmd_display_message_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if 0 != args_has(args, 70 as u_char) && (*args).argc != 0i32 {
        cmdq_error(item,
        ...
        if (*args).argc != 0i32 { template = *(*args).argv.offset(0isize) }
        ...
        c = cmd_find_client(item, args_get(args, 99 as u_char), 1i32);
        if c != 0 as *mut libc::c_void as *mut client && (*c).session == s {
            target_c = c
        } else { target_c = cmd_find_best_client(s) }
        ft = format_create((*item).client, item, 0i32, 0i32);
        format_defaults(ft, target_c, s, wl, wp);
        msg = format_expand_time(ft, template, time(0 as *mut time_t));
        if 0 != args_has((*self_0).args, 112 as u_char) {
...
pub static mut cmd_display_message_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                  ...
                  args:
                      unnamed_14{template:
                                     b"c:pt:F:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 1i32,},
                  usage:
                      b"[-p] [-c target-client] [-F format] [-t target-pane] [message]\x00"
                          as *const u8 as *const libc::c_char,
                  source:
