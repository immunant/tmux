...
pub const LINE_SEL_RIGHT_LEFT: unnamed_14 = 2;
#[derive ( Copy , Clone )]
#[repr ( C )]
pub struct args {
    pub tree: args_tree,
...
pub const LINE_SEL_NONE: unnamed_14 = 0;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const LINE_SEL_LEFT_RIGHT: unnamed_14 = 1;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
unsafe extern "C" fn cmd_copy_mode_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if 0 != args_has(args, 77 as u_char) {
        wp =
            cmd_mouse_pane(&mut (*shared).mouse as *mut mouse_event,
    ...
    if (*self_0).entry == &cmd_clock_mode_entry as *const cmd_entry {
        window_pane_set_mode(wp, &window_clock_mode as *const window_mode,
                             0 as *mut cmd_find_state, 0 as *mut args);
        return CMD_RETURN_NORMAL
    } else {
        if (*wp).mode != &window_copy_mode as *const window_mode {
            flag =
                window_pane_set_mode(wp,
        ...
        if 0 != args_has(args, 77 as u_char) {
            if (*wp).mode != 0 as *mut libc::c_void as *const window_mode &&
                ...
                window_copy_start_drag(c,
                                       &mut (*shared).mouse as
                                           *mut mouse_event);
            }
        }
        if (*wp).mode == &window_copy_mode as *const window_mode &&
               0 != args_has((*self_0).args, 117 as u_char) {
...
pub static mut cmd_clock_mode_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"clock-mode\x00" as *const u8 as *const libc::c_char,
...
pub static mut cmd_copy_mode_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"copy-mode\x00" as *const u8 as *const libc::c_char,
                  alias: 0 as *const libc::c_char,
                  args:
                      unnamed_27{template:
                                     b"Met:u\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-Mu] [-t target-pane]\x00" as *const u8 as
                          *const libc::c_char,
                  source:
                      cmd_entry_flag{flag: 0,
                                     type_0: CMD_FIND_PANE,
