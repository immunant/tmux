...
pub const JOB_CLOSED: unnamed_39 = 2;
...
pub const JOB_RUNNING: unnamed_39 = 0;
...
pub const JOB_DEAD: unnamed_39 = 1;
#[derive ( Copy , Clone )]
#[repr ( C )]
pub struct session {
    pub id: u_int,
...
pub type unnamed_39 = libc::c_uint;
...
pub unsafe extern "C" fn hooks_create(mut parent: *mut hooks) -> *mut hooks {
    ...
    hooks =
        xcalloc(1i32 as size_t,
                ::std::mem::size_of::<hooks>() as libc::c_ulong) as
            *mut hooks;
    loop  {
        let ref mut fresh0 =
            (*(&mut (*hooks).tree as *mut hooks_tree)).rbh_root;
        *fresh0 = 0 as *mut hook;
        if !(0 != 0i32) { break ; }
    }
    (*hooks).parent = parent;
    return hooks;
...
pub unsafe extern "C" fn hooks_free(mut hooks: *mut hooks) -> () {
    ...
    hook =
        hooks_tree_RB_MINMAX(&mut (*hooks).tree as *mut hooks_tree,
                             1i32.wrapping_neg());
    while hook != 0 as *mut libc::c_void as *mut hook &&
              { hook1 = hooks_tree_RB_NEXT(hook); 0 != 1i32 } {
...
unsafe extern "C" fn hooks_free1(mut hooks: *mut hooks, mut hook: *mut hook)
 -> () {
    hooks_tree_RB_REMOVE(&mut (*hooks).tree as *mut hooks_tree, hook);
    cmd_list_free((*hook).cmdlist);
    free((*hook).name as *mut libc::c_char as *mut libc::c_void);
    free(hook as *mut libc::c_void);
}
unsafe extern "C" fn hooks_tree_RB_REMOVE(mut head: *mut hooks_tree,
                                          mut elm: *mut hook) -> *mut hook {
    ...
    if (*elm).entry.rbe_left == 0 as *mut libc::c_void as *mut hook {
        child = (*elm).entry.rbe_right;
        current_block = 9386390421034826751;
    } else if (*elm).entry.rbe_right == 0 as *mut libc::c_void as *mut hook {
        ...
        } else { current_block = 5333453573631877616; }
    }
    match current_block {
        9386390421034826751 => {
            parent = (*elm).entry.rbe_parent;
    ...
    if color == 0i32 { hooks_tree_RB_REMOVE_COLOR(head, parent, child); }
    return old;
}
unsafe extern "C" fn hooks_tree_RB_REMOVE_COLOR(mut head: *mut hooks_tree,
                                                mut parent: *mut hook,
                                                mut elm: *mut hook) -> () {
    let mut current_block: u64;
    ...
    's_4:
        loop  {
            if !((elm == 0 as *mut libc::c_void as *mut hook ||
                    ...
                    match current_block {
                        ...
                        _ => {
                            if ((*tmp).entry.rbe_left ==
                                    0 as *mut libc::c_void as *mut hook ||
...
unsafe extern "C" fn hooks_tree_RB_MINMAX(mut head: *mut hooks_tree,
                                          mut val: libc::c_int) -> *mut hook {
...
pub unsafe extern "C" fn hooks_first(mut hooks: *mut hooks) -> *mut hook {
    return hooks_tree_RB_MINMAX(&mut (*hooks).tree as *mut hooks_tree,
                                1i32.wrapping_neg());
}
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn hooks_add(mut hooks: *mut hooks,
    ...
    hook =
        xcalloc(1i32 as size_t,
                ::std::mem::size_of::<hook>() as libc::c_ulong) as *mut hook;
    (*hook).name = xstrdup(name);
    (*hook).cmdlist = cmdlist;
    (*(*hook).cmdlist).references += 1;
    hooks_tree_RB_INSERT(&mut (*hooks).tree as *mut hooks_tree, hook);
}
unsafe extern "C" fn hooks_tree_RB_INSERT(mut head: *mut hooks_tree,
                                          mut elm: *mut hook) -> *mut hook {
    let mut tmp: *mut hook = 0 as *mut hook;
    let mut parent: *mut hook = 0 as *mut hook;
    ...
    if parent != 0 as *mut libc::c_void as *mut hook {
        if comp < 0i32 {
...
unsafe extern "C" fn hooks_tree_RB_INSERT_COLOR(mut head: *mut hooks_tree,
                                                mut elm: *mut hook) -> () {
    let mut current_block: u64;
    let mut parent: *mut hook = 0 as *mut hook;
...
unsafe extern "C" fn hooks_cmp(mut hook1: *mut hook, mut hook2: *mut hook)
 -> libc::c_int {
    return strcmp((*hook1).name, (*hook2).name);
}
unsafe extern "C" fn hooks_find1(mut hooks: *mut hooks,
                                 mut name: *const libc::c_char) -> *mut hook {
    let mut hook: hook =
        hook{name: 0 as *const libc::c_char,
             cmdlist: 0 as *mut cmd_list,
             entry:
                 unnamed_21{rbe_left: 0 as *mut hook,
                            rbe_right: 0 as *mut hook,
                            rbe_parent: 0 as *mut hook,
                            rbe_color: 0,},};
    hook.name = name;
    return hooks_tree_RB_FIND(&mut (*hooks).tree as *mut hooks_tree,
                              &mut hook as *mut hook);
}
unsafe extern "C" fn hooks_tree_RB_FIND(mut head: *mut hooks_tree,
                                        mut elm: *mut hook) -> *mut hook {
    let mut tmp: *mut hook = (*head).rbh_root;
    ...
    loop  {
        ...
        } else { return 0 as *mut hook }
    };
}
#[no_mangle]
pub unsafe extern "C" fn hooks_remove(mut hooks: *mut hooks,
                                      mut name: *const libc::c_char) -> () {
    let mut hook: *mut hook = 0 as *mut hook;
    hook = hooks_find1(hooks, name);
    if hook != 0 as *mut libc::c_void as *mut hook {
...
#[no_mangle]
pub unsafe extern "C" fn hooks_find(mut hooks: *mut hooks,
                                    mut name: *const libc::c_char)
 -> *mut hook {
    let mut hook0: hook =
        hook{name: 0 as *const libc::c_char,
    ...
    hook0.name = name;
    hook =
        hooks_tree_RB_FIND(&mut (*hooks).tree as *mut hooks_tree,
                           &mut hook0 as *mut hook);
    while hook == 0 as *mut libc::c_void as *mut hook {
        hooks = (*hooks).parent;
        if hooks == 0 as *mut libc::c_void as *mut hooks { break ; }
        hook =
            hooks_tree_RB_FIND(&mut (*hooks).tree as *mut hooks_tree,
                               &mut hook0 as *mut hook)
    }
...
unsafe extern "C" fn hooks_tree_RB_NFIND(mut head: *mut hooks_tree,
                                         mut elm: *mut hook) -> *mut hook {
    let mut tmp: *mut hook = (*head).rbh_root;
    let mut res: *mut hook = 0 as *mut hook;
    let mut comp: libc::c_int = 0;
    loop  {
        if !tmp.is_null() {
            comp = hooks_cmp(elm, tmp);
...
unsafe extern "C" fn hooks_tree_RB_PREV(mut elm: *mut hook) -> *mut hook {
