...
unsafe extern "C" fn cmd_list_sessions_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if template == 0 as *mut libc::c_void as *const libc::c_char {
        template =
            b"#{session_name}: #{session_windows} windows (created #{t:session_created}) [#{session_width}x#{session_height}]#{?session_grouped, (group ,}#{session_group}#{?session_grouped,),}#{?session_attached, (attached),}\x00"
