...
unsafe extern "C" fn cmd_list_windows_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    } else { cmd_list_windows_session(self_0, (*item).target.s, item, 0i32); }
...
unsafe extern "C" fn cmd_list_windows_session(mut self_0: *mut cmd,
    ...
    if template == 0 as *mut libc::c_void as *const libc::c_char {
        match type_0 {
            0 => {
                current_block = 7659877483494703492;
                match current_block {
                    12409452499708716668 => {
                        template =
            ...
            1 => {
                current_block = 12409452499708716668;
                match current_block {
                    12409452499708716668 => {
                        template =
                            b"#{session_name}:#{window_index}: #{window_name}#{window_flags} (#{window_panes} panes) [#{window_width}x#{window_height}] \x00"
...
pub static mut cmd_list_windows_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"list-windows\x00" as *const u8 as *const libc::c_char,
                  alias: b"lsw\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_35{template:
                                     b"F:at:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-a] [-F format] [-t target-session]\x00" as *const u8
