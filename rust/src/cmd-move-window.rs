 ...
 -> cmd_retval {
    ...
    if 0 != args_has(args, 114 as u_char) {
        if cmd_find_target(&mut (*item).target as *mut cmd_find_state, item,
                           tflag, CMD_FIND_SESSION, 2i32) != 0i32 {
    ...
    } else if cmd_find_target(&mut (*item).target as *mut cmd_find_state,
                              item, tflag, CMD_FIND_WINDOW, 4i32) != 0i32 {
        return CMD_RETURN_ERROR
    } else {
        src = (*item).source.s;
        dst = (*item).target.s;
        wl = (*item).source.wl;
        idx = (*item).target.idx;
        kflag = args_has((*self_0).args, 107 as u_char);
        dflag = args_has((*self_0).args, 100 as u_char);
        sflag = args_has((*self_0).args, 115 as u_char);
        if 0 != args_has((*self_0).args, 97 as u_char) {
            idx = winlink_shuffle_up(dst, (*dst).curw);
            if idx == 1i32.wrapping_neg() { return CMD_RETURN_ERROR }
        }
        if server_link_window(src, wl, dst, idx, kflag,
                              (0 == dflag) as libc::c_int,
                              &mut cause as *mut *mut libc::c_char) != 0i32 {
            cmdq_error(item,
                       b"can\'t link window: %s\x00" as *const u8 as
...
pub static mut cmd_move_window_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"move-window\x00" as *const u8 as *const libc::c_char,
                  alias: b"movew\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_19{template:
                                     b"adkrs:t:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-dkr] [-s src-window] [-t dst-window]\x00" as
...
pub static mut cmd_link_window_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"link-window\x00" as *const u8 as *const libc::c_char,
                  alias: b"linkw\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_19{template:
                                     b"adks:t:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-dk] [-s src-window] [-t dst-window]\x00" as
                          *const u8 as *const libc::c_char,
                  source:
                      cmd_entry_flag{flag: 115 as libc::c_char,
                                     type_0: CMD_FIND_WINDOW,
