...
pub unsafe extern "C" fn cmd_find_target(mut fs: *mut cmd_find_state,
 ...
 -> libc::c_int {
    ...
    } else { current_block = 15112282366752052665; }
    match current_block {
        7095457783677275021 => {
            if 0 == cmd_find_valid_state((*fs).current) {
                ...
                if target == 0 as *mut libc::c_void as *const libc::c_char ||
                    ...
                    } else { current_block = 17629765841327595216; }
                } else if strcmp(target,
                                 b"~\x00" as *const u8 as *const libc::c_char)
                              == 0i32 ||
                              strcmp(target,
                    ...
                    if 0 == server_check_marked() {
                        ...
                        cmd_find_copy_state(fs,
                                            &mut marked_pane as
                                                *mut cmd_find_state);
                        current_block = 17629765841327595216;
                    }
                } else {
                    copy = xstrdup(target);
                    ...
                    if colon != 0 as *mut libc::c_void as *mut libc::c_char &&
                        ...
                        match type_0 as libc::c_uint {
                            2 => {
                                current_block = 9316973474589727965;
                                match current_block {
                                    9316973474589727965 => { session = copy }
                                    10992171669449362944 => { pane = copy }
                                    _ => { window = copy }
                                }
                            }
                            1 => {
                                current_block = 674794500024072998;
                                match current_block {
                                    9316973474589727965 => { session = copy }
                                    10992171669449362944 => { pane = copy }
                                    _ => { window = copy }
                            ...
                            0 => {
                                current_block = 10992171669449362944;
                                match current_block {
                                    9316973474589727965 => { session = copy }
                                    10992171669449362944 => { pane = copy }
                                    _ => { window = copy }
                    ...
                    if pane != 0 as *mut libc::c_void as *const libc::c_char
                        ...
                        if session !=
                            ...
                            if cmd_find_get_session(fs, session) != 0i32 {
                                ...
                                if cmd_find_get_window_with_session(fs,
                                                                    window) !=
                                       0i32 {
                                    current_block = 3390465415055570287;
                                } else {
                                ...
                                if cmd_find_get_pane_with_session(fs, pane) !=
                                       0i32 {
                                    current_block = 4985680540995348867;
                                } else {
                                    current_block = 17629765841327595216;
                                }
                        ...
                        } else { current_block = 5374617794059532979; }
                        match current_block {
    ...
    if *session as libc::c_int == 36 {
        ...
        if (*fs).s != 0 as *mut libc::c_void as *mut session {
            ...
            if c != 0 as *mut libc::c_void as *mut client &&
                ...
                match current_block {
                    1394248824506584008 => { (*fs).s = s; return 0i32 }
                    _ => {
                        s = 0 as *mut session;
                        s_loop =
                            sessions_RB_MINMAX(&mut sessions as *mut sessions,
