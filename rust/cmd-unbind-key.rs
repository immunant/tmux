    ...
    if 0 == args_has(args, 97 as u_char) {
        ...
        cmdq_error(item,
                   b"key given with -a\x00" as *const u8 as
                       *const libc::c_char);
        return CMD_RETURN_ERROR
    } else { key = 281466386776064u64 }
    if key == 281466386776064u64 {
        tablename = args_get(args, 84 as u_char);
        if tablename == 0 as *mut libc::c_void as *const libc::c_char {
            key_bindings_remove_table(b"root\x00" as *const u8 as
                                          *const libc::c_char);
            key_bindings_remove_table(b"prefix\x00" as *const u8 as
                                          *const libc::c_char);
            return CMD_RETURN_NORMAL
        } else if key_bindings_get_table(tablename, 0i32) ==
        ...
        if 0 != args_has(args, 84 as u_char) {
            tablename = args_get(args, 84 as u_char);
            if key_bindings_get_table(tablename, 0i32) ==
                   0 as *mut libc::c_void as *mut key_table {
                cmdq_error(item,
                           b"table %s doesn\'t exist\x00" as *const u8 as
                               *const libc::c_char, tablename);
...
pub static mut cmd_unbind_key_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"unbind-key\x00" as *const u8 as *const libc::c_char,
                  alias: b"unbind\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_21{template:
                                     b"anT:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 1i32,},
                  usage:
                      b"[-an] [-T key-table] key\x00" as *const u8 as
                          *const libc::c_char,
                  source:
