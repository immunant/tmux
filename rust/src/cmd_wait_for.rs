...
pub struct wait_channel {
    ...
    pub waiters: unnamed_7,
    pub lockers: unnamed_0,
    pub entry: unnamed_1,
}
...
pub const PROMPT_ENTRY: unnamed_3 = 0;
#[derive ( Copy , Clone )]
#[repr ( C )]
pub struct window {
    pub id: u_int,
...
pub const PROMPT_COMMAND: unnamed_3 = 1;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub unsafe extern "C" fn cmd_wait_for_flush() -> () {
    ...
    while wc != 0 as *mut libc::c_void as *mut wait_channel &&
        ...
        wi = (*(&mut (*wc).waiters as *mut unnamed_7)).tqh_first;
        while wi != 0 as *mut libc::c_void as *mut wait_item &&
            ...
            loop  {
                if (*wi).entry.tqe_next !=
                    ...
                    let ref mut fresh0 =
                        (*(&mut (*wc).waiters as *mut unnamed_7)).tqh_last;
        ...
        (*wc).woken = 1i32;
        wi = (*(&mut (*wc).lockers as *mut unnamed_0)).tqh_first;
        while wi != 0 as *mut libc::c_void as *mut wait_item &&
                  { wi1 = (*wi).entry.tqe_next; 0 != 1i32 } {
            (*(*wi).item).flags &= !2i32;
            loop  {
                if (*wi).entry.tqe_next !=
                    ...
                    let ref mut fresh1 =
                        (*(&mut (*wc).lockers as *mut unnamed_0)).tqh_last;
...
unsafe extern "C" fn cmd_wait_for_remove(mut wc: *mut wait_channel) -> () {
    if 0 != (*wc).locked {
        return
                  ...
                  !((*(&mut (*wc).waiters as *mut unnamed_7)).tqh_first ==
 ...
 -> *mut wait_channel {
    ...
    if (*elm).entry.rbe_left == 0 as *mut libc::c_void as *mut wait_channel {
        ...
        if !parent.is_null() {
            ...
            current_block = 14379192954497154514;
        } else { current_block = 14379192954497154514; }
    }
    match current_block {
        9386390421034826751 => {
            parent = (*elm).entry.rbe_parent;
 ...
 -> cmd_retval {
    ...
    let mut wc0: wait_channel =
        wait_channel{name: 0 as *const libc::c_char,
                     ...
                     woken: 0,
                     waiters:
                         unnamed_7{tqh_first: 0 as *mut wait_item,
                                   tqh_last: 0 as *mut *mut wait_item,},
                     lockers:
                         unnamed_0{tqh_first: 0 as *mut wait_item,
                                   tqh_last: 0 as *mut *mut wait_item,},
                     entry:
                         unnamed_1{rbe_left: 0 as *mut wait_channel,
    ...
    loop  {
        ...
        *fresh4 = 0 as *mut wait_item;
        let ref mut fresh5 =
            (*(&mut (*wc).waiters as *mut unnamed_7)).tqh_last;
        *fresh5 =
            &mut (*(&mut (*wc).waiters as *mut unnamed_7)).tqh_first as
                *mut *mut wait_item;
        if !(0 != 0i32) { break ; }
    }
    loop  {
        let ref mut fresh6 =
            (*(&mut (*wc).lockers as *mut unnamed_0)).tqh_first;
        *fresh6 = 0 as *mut wait_item;
        let ref mut fresh7 =
            (*(&mut (*wc).lockers as *mut unnamed_0)).tqh_last;
        *fresh7 =
            &mut (*(&mut (*wc).lockers as *mut unnamed_0)).tqh_first as
                *mut *mut wait_item;
        if !(0 != 0i32) { break ; }
    }
...
unsafe extern "C" fn cmd_wait_for_signal(mut item: *mut cmdq_item,
 ...
 -> cmd_retval {
    ...
    if (*(&mut (*wc).waiters as *mut unnamed_7)).tqh_first ==
