...
#[no_mangle]
pub unsafe extern "C" fn control_notify_input(mut c: *mut client,
 ...
 -> () {
    ...
    if (*c).session == 0 as *mut libc::c_void as *mut session {
        ...
        buf = evbuffer_pullup(input, 1i32.wrapping_neg() as ssize_t);
        len = evbuffer_get_length(input);
        if winlink_find_by_window(&mut (*(*c).session).windows as
                                      *mut winlinks, (*wp).window) !=
            ...
            while (i as libc::c_ulong) < len {
                if (*buf.offset(i as isize) as libc::c_int) < 32 ||
                       *buf.offset(i as isize) as libc::c_int == 92 {
                    evbuffer_add_printf(message,
 ...
 -> () {
    ...
    while c != 0 as *mut libc::c_void as *mut client {
        if !(0 !=
            ...
            s = (*c).session;
            if !(winlink_find_by_window_id(&mut (*s).windows as *mut winlinks,
                                           (*w).id) ==
                     0 as *mut libc::c_void as *mut winlink) {
                if !((*w).layout_root ==
                         0 as *mut libc::c_void as *mut layout_cell) {
                    wl =
                        winlink_find_by_window(&mut (*s).windows as
                                                   *mut winlinks, w);
 ...
 -> () {
    ...
    while c != 0 as *mut libc::c_void as *mut client {
        if !(0 !=
                 ...
                 (*c).session == 0 as *mut libc::c_void as *mut session) {
            cs = (*c).session;
            if winlink_find_by_window_id(&mut (*cs).windows as *mut winlinks,
                                         (*w).id) !=
                   0 as *mut libc::c_void as *mut winlink {
                control_write(c,
...
pub unsafe extern "C" fn control_notify_client_session_changed(mut cc:
 ...
 -> () {
    let mut c: *mut client = 0 as *mut client;
    let mut s: *mut session = 0 as *mut session;
    if (*cc).session == 0 as *mut libc::c_void as *mut session {
        return
    } else {
        s = (*cc).session;
        c = (*(&mut clients as *mut clients)).tqh_first;
        while c != 0 as *mut libc::c_void as *mut client {
            if !(0 !=
                ...
                if cc == c {
                    control_write(c,
                                  b"%%session-changed $%u %s\x00" as *const u8
