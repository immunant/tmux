    ...
    if (*self_0).entry == &cmd_unlink_window_entry as *const cmd_entry {
        ...
        wl2 =
            winlinks_RB_MINMAX(&mut (*s).windows as *mut winlinks,
                               1i32.wrapping_neg());
        while wl2 != 0 as *mut libc::c_void as *mut winlink &&
                  { wl3 = winlinks_RB_NEXT(wl2); 0 != 1i32 } {
            if wl != wl2 { server_kill_window((*wl2).window); }
            wl2 = wl3
        }
    } else { server_kill_window((*wl).window); }
    recalculate_sizes();
...
pub static mut cmd_kill_window_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"kill-window\x00" as *const u8 as *const libc::c_char,
