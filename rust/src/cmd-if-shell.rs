...
unsafe extern "C" fn cmd_if_shell_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if 0 != args_has(args, 70 as u_char) {
        cmd = 0 as *mut libc::c_char;
        if *shellcmd as libc::c_int != 48 && *shellcmd as libc::c_int != 0 {
            cmd = *(*args).argv.offset(1isize)
        } else if (*args).argc == 3i32 { cmd = *(*args).argv.offset(2isize) }
        free(shellcmd as *mut libc::c_void);
        if cmd == 0 as *mut libc::c_void as *mut libc::c_char {
            return CMD_RETURN_NORMAL
        } else {
            cmdlist =
                cmd_string_parse(cmd, 0 as *const libc::c_char, 0i32 as u_int,
            ...
            if cmdlist == 0 as *mut libc::c_void as *mut cmd_list {
                ...
                new_item =
                    cmdq_get_command(cmdlist, 0 as *mut cmd_find_state,
                                     &mut (*shared).mouse as *mut mouse_event,
                                     0i32);
        ...
        cdata =
            xcalloc(1i32 as size_t,
                    ::std::mem::size_of::<cmd_if_shell_data>() as
                        libc::c_ulong) as *mut cmd_if_shell_data;
        if (*self_0).file != 0 as *mut libc::c_void as *mut libc::c_char {
        ...
        if (*cdata).client != 0 as *mut libc::c_void as *mut client {
            (*(*cdata).client).references += 1
        }
        if 0 == args_has(args, 98 as u_char) {
            (*cdata).item = item
        } else { (*cdata).item = 0 as *mut cmdq_item }
        memcpy(&mut (*cdata).mouse as *mut mouse_event as *mut libc::c_void,
               &mut (*shared).mouse as *mut mouse_event as
                   *const libc::c_void,
               ::std::mem::size_of::<mouse_event>() as libc::c_ulong);
        job_run(shellcmd, s, cwd, None, Some(cmd_if_shell_callback),
                Some(cmd_if_shell_free), cdata as *mut libc::c_void, 0i32);
        free(shellcmd as *mut libc::c_void);
        if 0 != args_has(args, 98 as u_char) {
            return CMD_RETURN_NORMAL
...
unsafe extern "C" fn cmd_if_shell_callback(mut job: *mut job) -> () {
    let mut cdata: *mut cmd_if_shell_data =
        (*job).data as *mut cmd_if_shell_data;
    ...
    if (0 !=
            !(unnamed_40{__in: (*job).status,}.__i & 127i32 == 0i32) as
                libc::c_int ||
            (unnamed_13{__in: (*job).status,}.__i & 65280i32) >> 8i32 != 0i32)
       {
        cmd = (*cdata).cmd_else
    } else { cmd = (*cdata).cmd_if }
    if !(cmd == 0 as *mut libc::c_void as *mut libc::c_char) {
        cmdlist =
            cmd_string_parse(cmd, file, line,
                             &mut cause as *mut *mut libc::c_char);
        if cmdlist == 0 as *mut libc::c_void as *mut cmd_list {
            if cause != 0 as *mut libc::c_void as *mut libc::c_char &&
                   (*cdata).item != 0 as *mut libc::c_void as *mut cmdq_item {
            ...
            new_item =
                cmdq_get_command(cmdlist, 0 as *mut cmd_find_state,
                                 &mut (*cdata).mouse as *mut mouse_event,
                                 0i32);
            cmd_list_free(cmdlist);
        }
        if new_item != 0 as *mut libc::c_void as *mut cmdq_item {
...
pub static mut cmd_if_shell_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"if-shell\x00" as *const u8 as *const libc::c_char,
                  alias: b"if\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_39{template:
                                     b"bFt:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 2i32,
                                 upper: 3i32,},
                  usage:
