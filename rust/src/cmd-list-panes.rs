...
unsafe extern "C" fn cmd_list_panes_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    } else if 0 != args_has(args, 115 as u_char) {
        cmd_list_panes_session(self_0, s, item, 1i32);
    } else { cmd_list_panes_window(self_0, s, wl, item, 0i32); }
...
unsafe extern "C" fn cmd_list_panes_window(mut self_0: *mut cmd,
    ...
    if template == 0 as *mut libc::c_void as *const libc::c_char {
        match type_0 {
            0 => {
                current_block = 9097293387489662194;
                match current_block {
            ...
            1 => {
                current_block = 11198266622936657545;
                match current_block {
                    11198266622936657545 => {
                        template =
                            b"#{window_index}.#{pane_index}: [#{pane_width}x#{pane_height}] [history #{history_size}/#{history_limit}, #{history_bytes} bytes] #{pane_id}#{?pane_active, (active),}#{?pane_dead, (dead),}\x00"
            ...
            2 => {
                current_block = 14365560136715140885;
                match current_block {
                    11198266622936657545 => {
                        template =
                            b"#{window_index}.#{pane_index}: [#{pane_width}x#{pane_height}] [history #{history_size}/#{history_limit}, #{history_bytes} bytes] #{pane_id}#{?pane_active, (active),}#{?pane_dead, (dead),}\x00"
...
pub static mut cmd_list_panes_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"list-panes\x00" as *const u8 as *const libc::c_char,
                  alias: b"lsp\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_37{template:
                                     b"asF:t:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-as] [-F format] [-t target-window]\x00" as *const u8
