...
pub const _ISlower: unnamed_20 = 512;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const _ISpunct: unnamed_20 = 4;
...
pub const _ISdigit: unnamed_20 = 2048;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const _ISgraph: unnamed_20 = 32768;
pub const _ISalnum: unnamed_20 = 8;
...
pub const _ISblank: unnamed_20 = 1;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const _ISupper: unnamed_20 = 256;
...
pub const _IScntrl: unnamed_20 = 2;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const _ISxdigit: unnamed_20 = 4096;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const _ISalpha: unnamed_20 = 1024;
#[derive ( Copy , Clone )]
#[repr ( C )]
pub struct tty {
    pub client: *mut client,
...
pub const _ISprint: unnamed_20 = 16384;
...
pub const _ISspace: unnamed_20 = 8192;
#[derive ( Copy , Clone )]
#[repr ( C )]
pub struct event {
    pub ev_active_next: unnamed_27,
...
pub unsafe extern "C" fn check_window_name(mut w: *mut window) -> () {
    ...
    if (*w).active == 0 as *mut libc::c_void as *mut window_pane {
        return
                  ...
                  options_get_number((*w).options,
                                     b"automatic-rename\x00" as *const u8 as
                                         *const libc::c_char) {
        return
    } else if 0 != !(*(*w).active).flags & 128i32 {
        ...
        left = name_time_expired(w, &mut tv as *mut timeval);
        if left != 0i32 {
            if 0 == event_initialized(&mut (*w).name_event as *mut event) {
                event_set(&mut (*w).name_event as *mut event,
                          1i32.wrapping_neg(), 0i32 as libc::c_short,
                          Some(name_time_callback), w as *mut libc::c_void);
            }
            if 0 ==
                   event_pending(&mut (*w).name_event as *mut event,
                                 1i32 as libc::c_short, 0 as *mut timeval) {
                log_debug(b"@%u name timer queued (%d left)\x00" as *const u8
                              as *const libc::c_char, (*w).id, left);
                let ref mut fresh0 = (*(&mut next as *mut timeval)).tv_usec;
                *fresh0 = 0i32 as __suseconds_t;
                (*(&mut next as *mut timeval)).tv_sec = *fresh0;
                next.tv_usec = left as __suseconds_t;
                event_add(&mut (*w).name_event as *mut event,
                          &mut next as *mut timeval);
            } else {
            ...
            if strcmp(name, (*w).name) != 0i32 {
                ...
                log_debug(b"@%u name not changed (still %s)\x00" as *const u8
                              as *const libc::c_char, (*w).id, (*w).name);
            }
 ...
 -> *mut libc::c_char {
    ...
    ft =
        format_create(0 as *mut client, 0 as *mut cmdq_item,
                      (1073741824u32 | (*w).id) as libc::c_int, 0i32);
...
unsafe extern "C" fn name_time_expired(mut w: *mut window,
    ...
    loop  {
        (*(&mut offset as *mut timeval)).tv_sec =
            (*tv).tv_sec - (*(&mut (*w).name_time as *mut timeval)).tv_sec;
        (*(&mut offset as *mut timeval)).tv_usec =
            (*tv).tv_usec - (*(&mut (*w).name_time as *mut timeval)).tv_usec;
        if (*(&mut offset as *mut timeval)).tv_usec < 0i32 as libc::c_long {
            let ref mut fresh1 = (*(&mut offset as *mut timeval)).tv_sec;
            *fresh1 -= 1;
            let ref mut fresh2 = (*(&mut offset as *mut timeval)).tv_usec;
            *fresh2 += 1000000i32 as libc::c_long
        }
        if !(0 != 0i32) { break ; }
    }
    if offset.tv_sec != 0i32 as libc::c_long ||
           offset.tv_usec > 500000i32 as libc::c_long {
        return 0i32
    } else {
        return (500000i32 as libc::c_long - offset.tv_usec) as libc::c_int
    };
}
#[no_mangle]
...
pub unsafe extern "C" fn parse_window_name(mut in_0: *const libc::c_char)
 -> *mut libc::c_char {
    ...
    while *name as libc::c_int == 32 || *name as libc::c_int == 45 {
    ...
    if *name as libc::c_int != 0 {
        ...
        while ptr > name &&
                  0 ==
                      *(*__ctype_b_loc()).offset(*ptr as u_char as libc::c_int
    ...
    if *name as libc::c_int == 47 { name = __xpg_basename(name) }
    name = xstrdup(name);
