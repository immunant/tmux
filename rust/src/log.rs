...
pub const PROMPT_COMMAND: unnamed_18 = 1;
...
pub const PROMPT_ENTRY: unnamed_18 = 0;
...
static mut log_level: libc::c_int = unsafe { 0 };
#[no_mangle]
...
pub unsafe extern "C" fn log_open(mut name: *const libc::c_char) -> () {
    ...
    if log_level == 0i32 {
        ...
        if log_file == 0 as *mut libc::c_void as *mut FILE {
            return
        } else {
            setvbuf(log_file, 0 as *mut libc::c_char, 1i32, 0i32 as size_t);
...
static mut log_file: *mut FILE = unsafe { 0 as *const FILE as *mut FILE };
#[no_mangle]
pub unsafe extern "C" fn log_close() -> () {
    if log_file != 0 as *mut libc::c_void as *mut FILE { fclose(log_file); }
...
#[no_mangle]
pub unsafe extern "C" fn log_vwrite(mut msg: *const libc::c_char,
    ...
    if log_file == 0 as *mut libc::c_void as *mut FILE {
        return
    } else if vasprintf(&mut fmt as *mut *mut libc::c_char, msg, ap) ==
                  1i32.wrapping_neg() {
        exit(1i32);
    } else if stravis(&mut out as *mut *mut libc::c_char, fmt,
                      1i32 | 2i32 | 8i32 | 16i32) == 1i32.wrapping_neg() {
        exit(1i32);
    } else {
        gettimeofday(&mut tv as *mut timeval, 0 as *mut timezone);
        if fprintf(log_file,
                   b"%lld.%06d %s\n\x00" as *const u8 as *const libc::c_char,
                   tv.tv_sec as libc::c_longlong, tv.tv_usec as libc::c_int,
                   out) == 1i32.wrapping_neg() {
            exit(1i32);
        } else {
            fflush(log_file);
