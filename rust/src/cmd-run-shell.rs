...
pub const PROMPT_COMMAND: unnamed_39 = 1;
#[derive ( Copy , Clone )]
#[repr ( C )]
pub struct event {
    pub ev_active_next: unnamed_30,
    pub ev_next: unnamed_2,
    pub ev_timeout_pos: unnamed_42,
...
pub const PROMPT_ENTRY: unnamed_39 = 0;
#[derive ( Copy , Clone )]
#[repr ( C )]
 ...
 -> cmd_retval {
    ...
    if (*item).client != 0 as *mut libc::c_void as *mut client &&
           (*(*item).client).session == 0 as *mut libc::c_void as *mut session
       {
        ...
        cwd = (*s).cwd
    } else { cwd = 0 as *const libc::c_char }
    cdata =
        xcalloc(1i32 as size_t,
                ::std::mem::size_of::<cmd_run_shell_data>() as libc::c_ulong)
            as *mut cmd_run_shell_data;
    (*cdata).cmd =
        format_single(item, *(*args).argv.offset(0isize), c, s, wl, wp);
    if 0 != args_has(args, 116 as u_char) &&
           wp != 0 as *mut libc::c_void as *mut window_pane {
        (*cdata).wp_id = (*wp).id as libc::c_int
    } else { (*cdata).wp_id = 1i32.wrapping_neg() }
    if 0 == args_has(args, 98 as u_char) { (*cdata).item = item }
    job_run((*cdata).cmd, s, cwd, None, Some(cmd_run_shell_callback),
            Some(cmd_run_shell_free), cdata as *mut libc::c_void, 0i32);
    if 0 != args_has(args, 98 as u_char) {
        return CMD_RETURN_NORMAL
    } else { return CMD_RETURN_WAIT };
}
...
unsafe extern "C" fn cmd_run_shell_callback(mut job: *mut job) -> () {
    let mut cdata: *mut cmd_run_shell_data =
        (*job).data as *mut cmd_run_shell_data;
    ...
    loop  {
        ...
        if !(line != 0 as *mut libc::c_void as *mut libc::c_char) { break ; }
    }
    size = evbuffer_get_length((*(*job).event).input);
    if size != 0i32 as libc::c_ulong {
        line =
            xmalloc(size.wrapping_add(1i32 as libc::c_ulong)) as
                *mut libc::c_char;
        memcpy(line as *mut libc::c_void,
               evbuffer_pullup((*(*job).event).input,
                               1i32.wrapping_neg() as ssize_t) as
                   *const libc::c_void, size);
        *line.offset(size as isize) = 0 as libc::c_char;
        cmd_run_shell_print(job, line);
        free(line as *mut libc::c_void);
    }
    msg = 0 as *mut libc::c_char;
    if (unnamed_29{__in: (*job).status,}.__i & 127i32 == 0i32) {
        retcode = (unnamed_4{__in: (*job).status,}.__i & 65280i32) >> 8i32;
        if retcode != 0i32 {
            xasprintf(&mut msg as *mut *mut libc::c_char,
                      b"\'%s\' returned %d\x00" as *const u8 as
                          *const libc::c_char, cmd, retcode);
        }
    } else if (((unnamed_36{__in: (*job).status,}.__i & 127i32) + 1i32) as
                   libc::c_schar as libc::c_int >> 1i32 > 0i32) {
        retcode = unnamed_21{__in: (*job).status,}.__i & 127i32;
        xasprintf(&mut msg as *mut *mut libc::c_char,
                  b"\'%s\' terminated by signal %d\x00" as *const u8 as
                      *const libc::c_char, cmd, retcode);
    }
    if msg != 0 as *mut libc::c_void as *mut libc::c_char {
        cmd_run_shell_print(job, msg);
    }
    free(msg as *mut libc::c_void);
    if (*cdata).item != 0 as *mut libc::c_void as *mut cmdq_item {
        (*(*cdata).item).flags &= !2i32
    };
}
unsafe extern "C" fn cmd_run_shell_print(mut job: *mut job,
    ...
    let mut fs: cmd_find_state =
        cmd_find_state{flags: 0,
                       ...
                       idx: 0,};
    if (*cdata).wp_id != 1i32.wrapping_neg() {
    ...
    if wp == 0 as *mut libc::c_void as *mut window_pane {
        if (*cdata).item != 0 as *mut libc::c_void as *mut cmdq_item {
            ...
            return
        } else if cmd_find_from_nothing(&mut fs as *mut cmd_find_state, 0i32)
                      != 0i32 {
            return
        } else {
            wp = fs.wp;
            if wp == 0 as *mut libc::c_void as *mut window_pane { return }
        }
    }
    if window_pane_set_mode(wp, &window_copy_mode as *const window_mode,
                            0 as *mut cmd_find_state, 0 as *mut args) == 0i32
       {
...
pub static mut cmd_run_shell_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"run-shell\x00" as *const u8 as *const libc::c_char,
                  ...
                  args:
                      unnamed_33{template:
                                 ...
                                 upper: 1i32,},
                  usage:
                      b"[-b] [-t target-pane] shell-command\x00" as *const u8
