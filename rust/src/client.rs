...
use proc_::{evbuffer, event_base, proc_send, proc_exit, proc_loop, proc_start, proc_set_signals, proc_clear_signals, proc_add_peer, tmuxpeer, tmuxproc};
use session::{session, sessions, session_group, session_groups};
...
pub struct sigaction {
    pub __sigaction_handler: unnamed_2,
...
pub const PROMPT_ENTRY: unnamed_41 = 0;
...
pub const CLIENT_EXIT_LOST_SERVER: unnamed_11 = 5;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const CLIENT_EXIT_EXITED: unnamed_11 = 6;
...
pub const CLIENT_EXIT_DETACHED: unnamed_11 = 1;
...
pub const CLIENT_EXIT_NONE: unnamed_11 = 0;
...
pub const CLIENT_EXIT_SERVER_EXITED: unnamed_11 = 7;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub const CLIENT_EXIT_LOST_TTY: unnamed_11 = 3;
...
pub const CLIENT_EXIT_TERMINATED: unnamed_11 = 4;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
pub unsafe extern "C" fn client_main(mut base: *mut event_base,
    ...
    if shell_command != 0 as *mut libc::c_void as *const libc::c_char {
        ...
        if cmdlist != 0 as *mut libc::c_void as *mut cmd_list {
            cmd = (*(&mut (*cmdlist).list as *mut unnamed_10)).tqh_first;
...
static mut client_exitreason: unnamed_11 = unsafe { CLIENT_EXIT_NONE };
...
static mut client_stdin: event =
    unsafe {
        event{ev_active_next:
                  unnamed_0{tqe_next: 0 as *const event as *mut event,
                            tqe_prev:
                                0 as *const *mut event as *mut *mut event,},
              ev_next:
                  unnamed_19{tqe_next: 0 as *const event as *mut event,
                             tqe_prev:
                                 0 as *const *mut event as *mut *mut event,},
              ev_timeout_pos:
...
unsafe extern "C" fn client_dispatch_attached(mut imsg: *mut imsg) -> () {
    let mut sigact: sigaction =
        sigaction{__sigaction_handler: unnamed_2{sa_handler: None,},
        ...
        loop  {
            ...
            if fd == 1i32.wrapping_neg() {
                ...
                log_debug(b"trying connect\x00" as *const u8 as
                              *const libc::c_char);
                if !(connect(fd, &mut sa as *mut sockaddr_un as *mut sockaddr,
...
unsafe extern "C" fn client_signal(mut sig: libc::c_int) -> () {
    ...
    if sig == 17i32 {
        ...
        match sig {
            ...
            28 => {
                current_block = 5302455684255568613;
                match current_block {
                    11557133005156592398 => {
                        client_exitreason = CLIENT_EXIT_LOST_TTY;
                        client_exitval = 1i32;
            ...
            18 => {
                current_block = 1350307133464837909;
                match current_block {
                    11557133005156592398 => {
                        client_exitreason = CLIENT_EXIT_LOST_TTY;
                        client_exitval = 1i32;
