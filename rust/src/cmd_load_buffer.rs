...
extern "C" {
    ...
    #[no_mangle]
    fn server_client_get_path(_: *mut client, _: *const libc::c_char)
     -> *mut libc::c_char;
    #[no_mangle]
    fn server_set_stdin_callback(_: *mut client,
                                 _:
...
unsafe extern "C" fn cmd_load_buffer_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if strcmp(path, b"-\x00" as *const u8 as *const libc::c_char) == 0i32 {
        ...
        error =
            server_set_stdin_callback(c, Some(cmd_load_buffer_callback),
        ...
        f = fopen(file, b"rb\x00" as *const u8 as *const libc::c_char);
        if f == 0 as *mut libc::c_void as *mut FILE {
            ...
            psize = 0i32 as size_t;
            loop  {
                ch = _IO_getc(f);
                if ch != 1i32.wrapping_neg() {
                    new_pdata =
                        realloc(pdata as *mut libc::c_void,
                                psize.wrapping_add(2i32 as libc::c_ulong)) as
                            *mut libc::c_char;
                    if new_pdata ==
                           0 as *mut libc::c_void as *mut libc::c_char {
                        cmdq_error(item,
                                   b"realloc error: %s\x00" as *const u8 as
                                       *const libc::c_char,
                                   strerror(*__errno_location()));
                        current_block = 2737365774328117838;
                        break ;
                    } else {
                        pdata = new_pdata;
                        let fresh0 = psize;
                    ...
                    match current_block {
                        2968425633554183086 => {
                            if pdata !=
                                   0 as *mut libc::c_void as *mut libc::c_char
                               {
                                *pdata.offset(psize as isize) =
                                    0 as libc::c_char
                            }
                            fclose(f);
                            free(file as *mut libc::c_void);
                            if paste_set(pdata, psize, bufname,
                                         &mut cause as *mut *mut libc::c_char)
                                   != 0i32 {
                                cmdq_error(item,
                        ...
                        _ => {
                            cmdq_error(item,
                                       b"%s: read error\x00" as *const u8 as
        ...
        free(pdata as *mut libc::c_void);
        if f != 0 as *mut libc::c_void as *mut FILE { fclose(f); }
        free(file as *mut libc::c_void);
        return CMD_RETURN_ERROR
    };
}
unsafe extern "C" fn cmd_load_buffer_callback(mut c: *mut client,
 ...
 -> () {
    ...
    if 0 == closed {
        ...
        if !(0 != (*c).flags & 512i32) {
            ...
            if !(psize == 0i32 as libc::c_ulong ||
                     {
                         pdata =
                             malloc(psize.wrapping_add(1i32 as libc::c_ulong))
                                 as *mut libc::c_char;
                         pdata == 0 as *mut libc::c_void as *mut libc::c_char
                     }) {
                memcpy(pdata as *mut libc::c_void,
                       evbuffer_pullup((*c).stdin_data,
                                       1i32.wrapping_neg() as ssize_t) as
                           *const libc::c_void, psize);
                *pdata.offset(psize as isize) = 0 as libc::c_char;
                evbuffer_drain((*c).stdin_data, psize);
                if paste_set(pdata, psize, (*cdata).bufname,
                             &mut cause as *mut *mut libc::c_char) != 0i32 {
                    if 0 != !(*c).flags & 65536i32 {
                        saved = cause;
                        cause = utf8_sanitize(saved);
                        free(saved as *mut libc::c_void);
                    }
                    evbuffer_add_printf((*c).stderr_data,
                                        b"%s\x00" as *const u8 as
                                            *const libc::c_char, cause);
...
pub static mut cmd_load_buffer_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"load-buffer\x00" as *const u8 as *const libc::c_char,
                  ...
                  args:
                      unnamed_20{template:
                                 ...
                                 upper: 1i32,},
                  usage:
                      b"[-b buffer-name] path\x00" as *const u8 as
                          *const libc::c_char,
                  source:
