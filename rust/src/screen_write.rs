...
unsafe extern "C" fn screen_write_collect_scroll(mut ctx:
 ...
 -> () {
    ...
    while y < (*s).rlower {
        ...
        loop  {
            if !((*(&mut (*cl).items as *mut unnamed_37)).tqh_first ==
                     0 as *mut libc::c_void as *mut screen_write_collect_item)
               {
                let ref mut fresh6 =
                    *(*(&mut (*(*ctx).list.offset(y as isize)).items as
                            *mut unnamed_37)).tqh_last;
                *fresh6 = (*(&mut (*cl).items as *mut unnamed_37)).tqh_first;
                let ref mut fresh7 =
                    (*(*(&mut (*cl).items as
                             *mut unnamed_37)).tqh_first).entry.tqe_prev;
                *fresh7 =
                    (*(&mut (*(*ctx).list.offset(y as isize)).items as
                           *mut unnamed_37)).tqh_last;
                let ref mut fresh8 =
                    (*(&mut (*(*ctx).list.offset(y as isize)).items as
                           *mut unnamed_37)).tqh_last;
                *fresh8 = (*(&mut (*cl).items as *mut unnamed_37)).tqh_last;
                loop  {
                    let ref mut fresh9 =
                        (*(&mut (*cl).items as *mut unnamed_37)).tqh_first;
                    *fresh9 = 0 as *mut screen_write_collect_item;
                    let ref mut fresh10 =
                        (*(&mut (*cl).items as *mut unnamed_37)).tqh_last;
                    *fresh10 =
                        &mut (*(&mut (*cl).items as
                                    *mut unnamed_37)).tqh_first as
                            *mut *mut screen_write_collect_item;
        ...
        loop  {
            ...
            *fresh11 = 0 as *mut screen_write_collect_item;
            let ref mut fresh12 =
                (*(&mut (*cl).items as *mut unnamed_37)).tqh_last;
            *fresh12 =
                &mut (*(&mut (*cl).items as *mut unnamed_37)).tqh_first as
                    *mut *mut screen_write_collect_item;
            if !(0 != 0i32) { break ; }
        }
        y = y.wrapping_add(1)
