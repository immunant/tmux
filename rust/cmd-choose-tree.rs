...
unsafe extern "C" fn cmd_choose_tree_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if (*self_0).entry == &cmd_choose_buffer_entry as *const cmd_entry {
        if paste_get_top(0 as *mut *const libc::c_char) ==
               0 as *mut libc::c_void as *mut paste_buffer {
            return CMD_RETURN_NORMAL
        } else { mode = &window_buffer_mode as *const window_mode }
    } else if (*self_0).entry == &cmd_choose_client_entry as *const cmd_entry
     {
        if server_client_how_many() == 0i32 as libc::c_uint {
            return CMD_RETURN_NORMAL
        } else { mode = &window_client_mode as *const window_mode }
    } else { mode = &window_tree_mode as *const window_mode }
    window_pane_set_mode(wp, mode, &mut (*item).target as *mut cmd_find_state,
                         args);
    return CMD_RETURN_NORMAL;
}
#[no_mangle]
pub static mut cmd_choose_client_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                  ...
                  args:
                      unnamed_12{template:
                                     b"F:f:NO:t:Z\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 1i32,},
                  usage:
                      b"[-N] [-F format] [-f filter] [-O sort-order] [-t target-pane]\x00"
...
pub static mut cmd_choose_tree_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"choose-tree\x00" as *const u8 as *const libc::c_char,
                  alias: 0 as *const libc::c_char,
                  args:
                      unnamed_12{template:
                                     b"F:Gf:NO:st:wZ\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 1i32,},
                  usage:
                      b"[-GNsw] [-F format] [-f filter] [-O sort-order] [-t target-pane]\x00"
                          as *const u8 as *const libc::c_char,
                  source:
                      cmd_entry_flag{flag: 0,
                                     type_0: CMD_FIND_PANE,
