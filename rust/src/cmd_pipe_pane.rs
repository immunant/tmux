...
unsafe extern "C" fn __sigismember(mut __set: *const __sigset_t,
                                   mut __sig: libc::c_int) -> libc::c_int {
    let mut __mask: libc::c_ulong =
            ...
            ((__sig - 1i32) as
                 libc::c_ulong).wrapping_rem((8i32 as
                                                  libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_ulong>()
                                                                                  as
                                                                                  libc::c_ulong));
    let mut __word: libc::c_ulong =
        ((__sig - 1i32) as
             libc::c_ulong).wrapping_div((8i32 as
                                              libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_ulong>()
                                                                              as
                                                                              libc::c_ulong));
    return if 0 != (*__set).__val[__word as usize] & __mask {
               1i32
           } else { 0i32 };
}
unsafe extern "C" fn __sigaddset(mut __set: *mut __sigset_t,
                                 mut __sig: libc::c_int) -> libc::c_int {
    let mut __mask: libc::c_ulong =
        (1i32 as libc::c_ulong) <<
            ((__sig - 1i32) as
        ...
        ((__sig - 1i32) as
             libc::c_ulong).wrapping_div((8i32 as
                                              libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_ulong>()
                                                                              as
                                                                              libc::c_ulong));
    (*__set).__val[__word as usize] |= __mask;
    return 0i32;
}
unsafe extern "C" fn __sigdelset(mut __set: *mut __sigset_t,
                                 mut __sig: libc::c_int) -> libc::c_int {
    let mut __mask: libc::c_ulong =
        (1i32 as libc::c_ulong) <<
            ((__sig - 1i32) as
...
unsafe extern "C" fn cmd_pipe_pane_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if (*args).argc == 0i32 ||
           **(*args).argv.offset(0isize) as libc::c_int == 0 {
        return CMD_RETURN_NORMAL
    } else if 0 != args_has((*self_0).args, 111 as u_char) &&
                  old_fd != 1i32.wrapping_neg() {
        return CMD_RETURN_NORMAL
    } else {
        if 0 != args_has((*self_0).args, 73 as u_char) {
            in_0 = 1i32;
            out = args_has((*self_0).args, 79 as u_char)
        } else { in_0 = 0i32; out = 1i32 }
        if socketpair(1i32, SOCK_STREAM as libc::c_int, 0i32,
                      pipe_fd.as_mut_ptr()) != 0i32 {
            cmdq_error(item,
            ...
            cmd =
                format_expand_time(ft, *(*args).argv.offset(0isize),
            ...
            match fork() {
                ...
                0 => {
                    ...
                    if 0 != out {
                        if dup2(pipe_fd[1usize], 0i32) == 1i32.wrapping_neg()
                           {
                            _exit(1i32);
                        }
                    } else if dup2(null_fd, 0i32) == 1i32.wrapping_neg() {
                        _exit(1i32);
                    }
                    if 0 != in_0 {
                        if dup2(pipe_fd[1usize], 1i32) == 1i32.wrapping_neg()
                           {
                            _exit(1i32);
                        } else if pipe_fd[1usize] != 1i32 {
                            close(pipe_fd[1usize]);
                        }
                    } else if dup2(null_fd, 1i32) == 1i32.wrapping_neg() {
                        _exit(1i32);
                    }
                    if dup2(null_fd, 2i32) == 1i32.wrapping_neg() {
                        _exit(1i32);
                    } else {
                        closefrom(2i32 + 1i32);
                ...
                _ => {
                    ...
                    (*wp).pipe_off =
                        evbuffer_get_length((*(*wp).event).input);
...
unsafe extern "C" fn cmd_pipe_pane_read_callback(mut bufev: *mut bufferevent,
 ...
 -> () {
    ...
    bufferevent_write((*wp).event,
                      evbuffer_pullup(evb, 1i32.wrapping_neg() as ssize_t) as
...
pub static mut cmd_pipe_pane_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"pipe-pane\x00" as *const u8 as *const libc::c_char,
                  alias: b"pipep\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_5{template:
                                    b"IOot:\x00" as *const u8 as
                                        *const libc::c_char,
                                lower: 0i32,
                                upper: 1i32,},
                  usage:
                      b"[-IOo] [-t target-pane] [command]\x00" as *const u8 as
                          *const libc::c_char,
                  source:
