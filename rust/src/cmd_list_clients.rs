...
unsafe extern "C" fn cmd_list_clients_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if template == 0 as *mut libc::c_void as *const libc::c_char {
        template =
            b"#{client_name}: #{session_name} [#{client_width}x#{client_height} #{client_termname}]#{?client_utf8, (utf8),} #{?client_readonly, (ro),}\x00"
...
pub static mut cmd_list_clients_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"list-clients\x00" as *const u8 as *const libc::c_char,
                  alias: b"lsc\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_10{template:
                                     b"F:t:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-F format] [-t target-session]\x00" as *const u8 as
                          *const libc::c_char,
                  source:
