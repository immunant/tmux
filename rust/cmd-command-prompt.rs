...
unsafe extern "C" fn cmd_command_prompt_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    let mut n: size_t = 0;
    c = cmd_find_client(item, args_get(args, 116 as u_char), 0i32);
    if c == 0 as *mut libc::c_void as *mut client {
        ...
        prompts = args_get(args, 112 as u_char);
        if prompts != 0 as *mut libc::c_void as *const libc::c_char {
        ...
        ptr =
            strsep(&mut (*cdata).next_prompt as *mut *mut libc::c_char,
                   b",\x00" as *const u8 as *const libc::c_char);
        if prompts == 0 as *mut libc::c_void as *const libc::c_char {
            prompt = xstrdup(ptr)
        } else {
        ...
        inputs = args_get(args, 73 as u_char);
        if inputs != 0 as *mut libc::c_void as *const libc::c_char {
            (*cdata).inputs = xstrdup(inputs);
            (*cdata).next_input = (*cdata).inputs;
            input =
                strsep(&mut (*cdata).next_input as *mut *mut libc::c_char,
                       b",\x00" as *const u8 as *const libc::c_char)
        }
        if 0 != args_has(args, 49 as u_char) {
            (*cdata).flags |= 1i32
        } else if 0 != args_has(args, 78 as u_char) {
            (*cdata).flags |= 2i32
        } else if 0 != args_has(args, 105 as u_char) {
            (*cdata).flags |= 4i32
        }
        status_prompt_set(c, prompt, input, Some(cmd_command_prompt_callback),
...
unsafe extern "C" fn cmd_command_prompt_callback(mut c: *mut client,
 ...
 -> libc::c_int {
    ...
    if s == 0 as *mut libc::c_void as *const libc::c_char {
        return 0i32
    } else if 0 != done && 0 != (*cdata).flags & 4i32 {
        return 0i32
    } else {
        new_template =
            cmd_template_replace((*cdata).template, s, (*cdata).idx);
        if 0 != done {
            free((*cdata).template as *mut libc::c_void);
        ...
        if 0 != done &&
               {
                   ptr =
                       strsep(&mut (*cdata).next_prompt as
                                  *mut *mut libc::c_char,
                              b",\x00" as *const u8 as *const libc::c_char);
            ...
            cmdlist =
                cmd_string_parse(new_template, 0 as *const libc::c_char,
                                 0i32 as u_int,
            ...
            if cmdlist == 0 as *mut libc::c_void as *mut cmd_list {
                if cause != 0 as *mut libc::c_void as *mut libc::c_char {
                    new_item =
                        cmdq_get_callback1(b"cmd_command_prompt_error\x00" as
                                               *const u8 as
                                               *const libc::c_char,
                                           Some(cmd_command_prompt_error),
                                           cause as *mut libc::c_void)
                } else { new_item = 0 as *mut cmdq_item }
            } else {
                new_item =
            ...
            if 0 == done { free(new_template as *mut libc::c_void); }
...
unsafe extern "C" fn cmd_command_prompt_error(mut item: *mut cmdq_item,
                                              mut data: *mut libc::c_void)
 -> cmd_retval {
...
pub static mut cmd_command_prompt_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                  ...
                  args:
                      unnamed_9{template:
                                    b"1iI:Np:t:\x00" as *const u8 as
                                        *const libc::c_char,
                                lower: 0i32,
                                upper: 1i32,},
                  usage:
                      b"[-1Ni] [-I inputs] [-p prompts] [-t target-client] [template]\x00"
