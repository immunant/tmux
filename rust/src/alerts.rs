        ...
        if 0 == (*w).alerts_queued {
            ...
            loop  {
                ...
                (*w).alerts_entry.tqe_prev =
                    (*(&mut alerts_list as *mut unnamed_38)).tqh_last;
                let ref mut fresh1 =
                    *(*(&mut alerts_list as *mut unnamed_38)).tqh_last;
                *fresh1 = w;
                let ref mut fresh2 =
...
unsafe extern "C" fn alerts_callback(mut fd: libc::c_int,
    ...
    w = (*(&mut alerts_list as *mut unnamed_38)).tqh_first;
    while w != 0 as *mut libc::c_void as *mut window &&
...
pub static mut alerts_list: unnamed_38 =
    unsafe {
        unnamed_38{tqh_first: 0 as *const window as *mut window,
                   tqh_last: 0 as *const *mut window as *mut *mut window,} // Initialized in main
    };
...
unsafe extern "C" fn alerts_check_silence(mut w: *mut window) -> libc::c_int {
    ...
    if 0 != !(*w).flags & 8i32 {
        ...
        wl = (*(&mut (*w).winlinks as *mut unnamed_18)).tqh_first;
