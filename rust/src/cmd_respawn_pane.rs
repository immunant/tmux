 ...
 -> cmd_retval {
    ...
    if 0 == args_has((*self_0).args, 107 as u_char) &&
        ...
        if window_pane_index(wp, &mut idx as *mut u_int) != 0i32 {
            fatalx(b"index not found\x00" as *const u8 as
                       *const libc::c_char);
        } else {
            cmdq_error(item,
                       b"pane still active: %s:%d.%u\x00" as *const u8 as
                           *const libc::c_char, (*s).name, (*wl).idx, idx);
...
pub static mut cmd_respawn_pane_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"respawn-pane\x00" as *const u8 as *const libc::c_char,
                  ...
                  args:
                      unnamed_24{template:
                                 ...
                                 upper: -1i32,},
                  usage:
                      b"[-c start-directory] [-k] [-t target-pane] [command]\x00"
