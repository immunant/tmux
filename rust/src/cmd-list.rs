...
pub unsafe extern "C" fn cmd_list_parse(mut argc: libc::c_int,
 ...
 -> *mut cmd_list {
    ...
    loop  {
        let ref mut fresh0 =
            (*(&mut (*cmdlist).list as *mut unnamed_6)).tqh_first;
        *fresh0 = 0 as *mut cmd;
        let ref mut fresh1 =
            (*(&mut (*cmdlist).list as *mut unnamed_6)).tqh_last;
        *fresh1 =
            &mut (*(&mut (*cmdlist).list as *mut unnamed_6)).tqh_first as
                *mut *mut cmd;
        if !(0 != 0i32) { break ; }
    }
    lastsplit = 0i32;
...
pub unsafe extern "C" fn cmd_list_free(mut cmdlist: *mut cmd_list) -> () {
    ...
    if (*cmdlist).references != 0i32 {
        ...
        cmd = (*(&mut (*cmdlist).list as *mut unnamed_6)).tqh_first;
