...
pub const LINE_SEL_RIGHT_LEFT: unnamed_18 = 2;
...
pub const LINE_SEL_LEFT_RIGHT: unnamed_18 = 1;
...
pub const LINE_SEL_NONE: unnamed_18 = 0;
#[derive ( Copy , Clone )]
#[repr ( C )]
...
unsafe extern "C" fn cmd_set_buffer_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if (*self_0).entry == &cmd_delete_buffer_entry as *const cmd_entry {
        ...
        } else { paste_free(pb); return CMD_RETURN_NORMAL }
    } else if 0 != args_has(args, 110 as u_char) {
        if pb == 0 as *mut libc::c_void as *mut paste_buffer {
            pb = paste_get_top(&mut bufname as *mut *const libc::c_char)
        }
        if pb == 0 as *mut libc::c_void as *mut paste_buffer {
            cmdq_error(item,
                       b"no buffer\x00" as *const u8 as *const libc::c_char);
            return CMD_RETURN_ERROR
        } else if paste_rename(bufname, args_get(args, 110 as u_char),
        ...
        if newsize == 0i32 as libc::c_ulong {
            return CMD_RETURN_NORMAL
        } else {
            bufsize = 0i32 as size_t;
            bufdata = 0 as *mut libc::c_char;
            if 0 != args_has(args, 97 as u_char) &&
                   pb != 0 as *mut libc::c_void as *mut paste_buffer {
                olddata = paste_buffer_data(pb, &mut bufsize as *mut size_t);
                bufdata = xmalloc(bufsize) as *mut libc::c_char;
                memcpy(bufdata as *mut libc::c_void,
                       olddata as *const libc::c_void, bufsize);
            }
            bufdata =
                xrealloc(bufdata as *mut libc::c_void,
                         bufsize.wrapping_add(newsize)) as *mut libc::c_char;
            memcpy(bufdata.offset(bufsize as isize) as *mut libc::c_void,
                   *(*args).argv.offset(0isize) as *const libc::c_void,
                   newsize);
            bufsize =
                (bufsize as libc::c_ulong).wrapping_add(newsize) as size_t as
                    size_t;
            if paste_set(bufdata, bufsize, bufname,
                         &mut cause as *mut *mut libc::c_char) != 0i32 {
                cmdq_error(item,
...
pub static mut cmd_set_buffer_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"set-buffer\x00" as *const u8 as *const libc::c_char,
                  alias: b"setb\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_15{template:
                                     b"ab:n:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 1i32,},
                  usage:
                      b"[-a] [-b buffer-name] [-n new-buffer-name] data\x00"
