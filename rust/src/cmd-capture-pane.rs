 ...
 -> cmd_retval {
    ...
    if (*self_0).entry == &cmd_clear_history_entry as *const cmd_entry {
        ...
        if 0 != args_has(args, 80 as u_char) {
            buf = cmd_capture_pane_pending(args, wp, &mut len as *mut size_t)
        } else {
            buf =
                cmd_capture_pane_history(args, item, wp,
                                         &mut len as *mut size_t)
        }
        if buf == 0 as *mut libc::c_void as *mut libc::c_char {
            return CMD_RETURN_ERROR
        } else {
            if 0 != args_has(args, 112 as u_char) {
                c = (*item).client;
                if c == 0 as *mut libc::c_void as *mut client ||
                       (*c).session != 0 as *mut libc::c_void as *mut session
                           && 0 == (*c).flags & 8192i32 {
                    cmdq_error(item,
                               b"can\'t write to stdout\x00" as *const u8 as
                                   *const libc::c_char);
                    free(buf as *mut libc::c_void);
                    return CMD_RETURN_ERROR
                } else {
                    evbuffer_add((*c).stdout_data, buf as *const libc::c_void,
                                 len);
                    free(buf as *mut libc::c_void);
                    if 0 != args_has(args, 80 as u_char) &&
                           len > 0i32 as libc::c_ulong {
                ...
                if paste_set(buf, len, bufname,
                             &mut cause as *mut *mut libc::c_char) != 0i32 {
                    cmdq_error(item,
...
unsafe extern "C" fn cmd_capture_pane_history(mut args: *mut args,
 ...
 -> *mut libc::c_char {
    ...
    sx = (*(*(&mut (*wp).base as *mut screen)).grid).sx;
    if 0 != args_has(args, 97 as u_char) {
        gd = (*wp).saved_grid;
        if gd == 0 as *mut libc::c_void as *mut grid {
    ...
    Sflag = args_get(args, 83 as u_char);
    if Sflag != 0 as *mut libc::c_void as *const libc::c_char &&
           strcmp(Sflag, b"-\x00" as *const u8 as *const libc::c_char) == 0i32
       {
        top = 0i32 as u_int
    } else {
        n =
            args_strtonum(args, 83 as u_char,
                          (2147483647i32.wrapping_neg() - 1i32) as
                              libc::c_longlong, 32767i32 as libc::c_longlong,
                          &mut cause as *mut *mut libc::c_char) as
                libc::c_int;
        if cause != 0 as *mut libc::c_void as *mut libc::c_char {
            top = (*gd).hsize;
            free(cause as *mut libc::c_void);
        } else if n < 0i32 && n.wrapping_neg() as u_int > (*gd).hsize {
            top = 0i32 as u_int
    ...
    Eflag = args_get(args, 69 as u_char);
    if Eflag != 0 as *mut libc::c_void as *const libc::c_char &&
           strcmp(Eflag, b"-\x00" as *const u8 as *const libc::c_char) == 0i32
       {
        bottom =
            (*gd).hsize.wrapping_add((*gd).sy).wrapping_sub(1i32 as
                                                                libc::c_uint)
        ...
        n =
            args_strtonum(args, 69 as u_char,
                          (2147483647i32.wrapping_neg() - 1i32) as
                              libc::c_longlong, 32767i32 as libc::c_longlong,
                          &mut cause as *mut *mut libc::c_char) as
                libc::c_int;
    ...
    with_codes = args_has(args, 101 as u_char);
    escape_c0 = args_has(args, 67 as u_char);
    join_lines = args_has(args, 74 as u_char);
    buf = 0 as *mut libc::c_char;
    i = top;
    while i <= bottom {
        line =
            grid_string_cells(gd, 0i32 as u_int, i, sx,
                              &mut gc as *mut *mut grid_cell, with_codes,
                              escape_c0, (0 == join_lines) as libc::c_int);
        linelen = strlen(line);
        buf = cmd_capture_pane_append(buf, len, line, linelen);
        gl = grid_peek_line(gd, i);
        if 0 == join_lines || 0 == (*gl).flags & 1i32 {
            let fresh0 = *len;
            *len = (*len).wrapping_add(1);
            *buf.offset(fresh0 as isize) = 10 as libc::c_char
        }
 ...
 -> *mut libc::c_char {
    buf =
        xrealloc(buf as *mut libc::c_void,
                 (*len).wrapping_add(linelen).wrapping_add(1i32 as
                                                               libc::c_ulong))
            as *mut libc::c_char;
...
unsafe extern "C" fn cmd_capture_pane_pending(mut args: *mut args,
 ...
 -> *mut libc::c_char {
    ...
    if pending == 0 as *mut libc::c_void as *mut evbuffer {
        return xstrdup(b"\x00" as *const u8 as *const libc::c_char)
    } else {
        line =
            evbuffer_pullup(pending, 1i32.wrapping_neg() as ssize_t) as
                *mut libc::c_char;
        linelen = evbuffer_get_length(pending);
        buf = xstrdup(b"\x00" as *const u8 as *const libc::c_char);
        if 0 != args_has(args, 67 as u_char) {
            i = 0i32 as u_int;
            while (i as libc::c_ulong) < linelen {
                if *line.offset(i as isize) as libc::c_int >= 32 &&
                       *line.offset(i as isize) as libc::c_int != 92 {
...
pub static mut cmd_capture_pane_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"capture-pane\x00" as *const u8 as *const libc::c_char,
                  alias: b"capturep\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_2{template:
                                    b"ab:CeE:JpPqS:t:\x00" as *const u8 as
                                        *const libc::c_char,
                                lower: 0i32,
                                upper: 0i32,},
                  usage:
                      b"[-aCeJpPq] [-b buffer-name] [-E end-line] [-S start-line][-t target-pane]\x00"
