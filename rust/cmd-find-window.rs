...
unsafe extern "C" fn cmd_find_window_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    C = args_has(args, 67 as u_char);
    N = args_has(args, 78 as u_char);
    T = args_has(args, 84 as u_char);
    if 0 == C && 0 == N && 0 == T { T = 1i32; N = T; C = N }
    if 0 != C && 0 != N && 0 != T {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{||:#{C:%s},#{||:#{m:*%s*,#{window_name}},#{m:*%s*,#{pane_title}}}}\x00"
                      as *const u8 as *const libc::c_char, s, s, s);
    } else if 0 != C && 0 != N {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{||:#{C:%s},#{m:*%s*,#{window_name}}}\x00" as *const u8
                      as *const libc::c_char, s, s);
    } else if 0 != C && 0 != T {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{||:#{C:%s},#{m:*%s*,#{pane_title}}}\x00" as *const u8 as
                      *const libc::c_char, s, s);
    } else if 0 != N && 0 != T {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{||:#{m:*%s*,#{window_name}},#{m:*%s*,#{pane_title}}}\x00"
                      as *const u8 as *const libc::c_char, s, s);
    } else if 0 != C {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{C:%s}\x00" as *const u8 as *const libc::c_char, s);
    } else if 0 != N {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{m:*%s*,#{window_name}}\x00" as *const u8 as
                      *const libc::c_char, s);
    } else {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{m:*%s*,#{pane_title}}\x00" as *const u8 as
                      *const libc::c_char, s);
    }
    new_args =
        args_parse(b"\x00" as *const u8 as *const libc::c_char, 1i32,
                   &mut argv as *mut *mut libc::c_char);
    args_set(new_args, 102 as u_char, filter);
    window_pane_set_mode(wp, &window_tree_mode as *const window_mode,
...
pub static mut cmd_find_window_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"find-window\x00" as *const u8 as *const libc::c_char,
                  alias: b"findw\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_37{template:
                                     b"CNt:T\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 1i32,
                                 upper: 1i32,},
                  usage:
                      b"[-CNT] [-t target-pane] match-string\x00" as *const u8
