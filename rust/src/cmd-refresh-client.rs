 ...
 -> cmd_retval {
    ...
    if c == 0 as *mut libc::c_void as *mut client {
        ...
        if 0 != args_has(args, 67 as u_char) {
            size = args_get(args, 67 as u_char);
            if size == 0 as *mut libc::c_void as *const libc::c_char {
                cmdq_error(item,
                           b"missing size\x00" as *const u8 as
                               *const libc::c_char);
                return CMD_RETURN_ERROR
            } else if sscanf(size,
                             b"%u,%u\x00" as *const u8 as *const libc::c_char,
                             &mut w as *mut u_int, &mut h as *mut u_int) !=
                          2i32 {
                cmdq_error(item,
                           b"bad size argument\x00" as *const u8 as
                               *const libc::c_char);
                return CMD_RETURN_ERROR
            } else if w < 2i32 as libc::c_uint || w > 5000i32 as libc::c_uint
                          || h < 2i32 as libc::c_uint ||
                          h > 5000i32 as libc::c_uint {
                cmdq_error(item,
                ...
                tty_set_size(&mut (*c).tty as *mut tty, w, h);
                (*c).flags |= 4194304i32;
                recalculate_sizes();
            }
        } else if 0 != args_has(args, 83 as u_char) {
            (*c).flags |= 524288i32;
            server_status_client(c);
        } else { (*c).flags |= 524288i32; server_redraw_client(c); }
    ...
    unsafe {
        cmd_entry{name:
                      b"refresh-client\x00" as *const u8 as
                          *const libc::c_char,
                  alias: b"refresh\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_10{template:
                                     b"C:St:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
                      b"[-S] [-C size] [-t target-client]\x00" as *const u8 as
                          *const libc::c_char,
                  source:
