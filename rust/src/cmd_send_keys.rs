...
unsafe extern "C" fn cmd_send_keys_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    let mut m: *mut mouse_event =
        &mut (*(*item).shared).mouse as *mut mouse_event;
    ...
    let mut np: u_int = 1i32 as u_int;
    let mut cause: *mut libc::c_char = 0 as *mut libc::c_char;
    if 0 != args_has(args, 78 as u_char) {
        np =
            args_strtonum(args, 78 as u_char, 1i32 as libc::c_longlong,
                          (2147483647i32 as
                               libc::c_uint).wrapping_mul(2u32).wrapping_add(1u32)
                              as libc::c_longlong,
                          &mut cause as *mut *mut libc::c_char) as u_int;
        ...
        } else if 0 != args_has(args, 88 as u_char) || (*args).argc == 0i32 {
            (*wp).modeprefix = np
        }
    }
    if 0 != args_has(args, 88 as u_char) {
        if (*wp).mode == 0 as *mut libc::c_void as *const window_mode ||
                                                                   ...
                                                                   *mut libc::c_void)
           {
            ...
            if 0 == (*m).valid {
                (*(*wp).mode).command.expect("non-null function pointer")(wp,
                                                                          c,
                                                                          s,
                                                                          args,
                                                                          0 as
    ...
    } else if 0 != args_has(args, 77 as u_char) {
        wp =
            cmd_mouse_pane(m, &mut s as *mut *mut session,
    ...
    } else if (*self_0).entry == &cmd_send_prefix_entry as *const cmd_entry {
        if 0 != args_has(args, 50 as u_char) {
            key =
        ...
        if 0 != args_has(args, 82 as u_char) {
        ...
        while np != 0i32 as libc::c_uint {
            i = 0i32;
            while i < (*args).argc {
                literal = args_has(args, 108 as u_char);
                if 0 == literal {
                    key =
                        key_string_lookup_string(*(*args).argv.offset(i as
                                                                          isize));
                    if key != 281470681743360u64 && key != 281466386776064u64
                       {
                        cmd_send_keys_inject(c, item, key);
                    } else { literal = 1i32 }
                }
                if 0 != literal {
                    ...
                    uc = ud;
                    while (*uc).size as libc::c_int != 0i32 {
                        if !(utf8_combine(uc, &mut wc as *mut wchar_t) as
                                 libc::c_uint !=
...
unsafe extern "C" fn cmd_send_keys_inject(mut c: *mut client,
                                          mut item: *mut cmdq_item,
                                          mut key: key_code) -> () {
    let mut wp: *mut window_pane = (*item).target.wp;
    let mut s: *mut session = (*item).target.s;
    ...
    let mut bd_find: key_binding =
        key_binding{key: 0,
                    ...
                    flags: 0,
                    entry:
                        unnamed_22{rbe_left: 0 as *mut key_binding,
    ...
    if (*wp).mode == 0 as *mut libc::c_void as *const window_mode ||
                                                                                ...
                                                                                *mut libc::c_void)
       {
        ...
        if bd != 0 as *mut libc::c_void as *mut key_binding {
            ...
            key_bindings_dispatch(bd, item, c, 0 as *mut mouse_event,
...
pub static mut cmd_send_prefix_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"send-prefix\x00" as *const u8 as *const libc::c_char,
                  alias: 0 as *const libc::c_char,
                  args:
                      unnamed_15{template:
                                     b"2t:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: 0i32,},
                  usage:
...
pub static mut cmd_send_keys_entry: cmd_entry =
    unsafe {
        cmd_entry{name: b"send-keys\x00" as *const u8 as *const libc::c_char,
                  alias: b"send\x00" as *const u8 as *const libc::c_char,
                  args:
                      unnamed_15{template:
                                     b"lXRMN:t:\x00" as *const u8 as
                                         *const libc::c_char,
                                 lower: 0i32,
                                 upper: -1i32,},
                  usage:
                      b"[-lXRM] [-N repeat-count] [-t target-pane] key ...\x00"
