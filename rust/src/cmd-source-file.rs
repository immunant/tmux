...
extern "C" {
    ...
    #[no_mangle]
    fn load_cfg(_: *const libc::c_char, _: *mut client, _: *mut cmdq_item,
                _: libc::c_int) -> libc::c_int;
    #[no_mangle]
...
unsafe extern "C" fn cmd_source_file_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    let mut quiet: libc::c_int = args_has(args, 113 as u_char);
    ...
    let mut path: *const libc::c_char = *(*args).argv.offset(0isize);
    let mut g: glob_t =
        glob_t{gl_pathc: 0,
               ...
               gl_stat: None,};
    let mut i: u_int = 0;
    if *path as libc::c_int == 47 {
        pattern = xstrdup(path)
    } else {
        utf8_stravis(&mut tmp as *mut *mut libc::c_char,
                     server_client_get_cwd(c), 256i32);
        xasprintf(&mut pattern as *mut *mut libc::c_char,
                  b"%s/%s\x00" as *const u8 as *const libc::c_char, tmp,
                  path);
    ...
    if glob(pattern, 0i32, None, &mut g as *mut glob_t) != 0i32 {
        if 0 == quiet || *__errno_location() != 2i32 {
            cmdq_error(item,
        ...
        while i < g.gl_pathc as u_int {
            if load_cfg(*g.gl_pathv.offset(i as isize), c, item, quiet) < 0i32
               {
                retval = CMD_RETURN_ERROR
            }
            i = i.wrapping_add(1)
        ...
        if 0 != cfg_finished {
            new_item =
                cmdq_get_callback1(b"cmd_source_file_done\x00" as *const u8 as
                                       *const libc::c_char,
...
unsafe extern "C" fn cmd_source_file_done(mut item: *mut cmdq_item,
...
pub static mut cmd_source_file_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"source-file\x00" as *const u8 as *const libc::c_char,
                  ...
                  args:
                      unnamed_6{template:
                                ...
                                lower: 1i32,
                                upper: 1i32,},
                  usage: b"[-q] path\x00" as *const u8 as *const libc::c_char,
                  source:
