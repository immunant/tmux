...
pub unsafe extern "C" fn recalculate_sizes() -> () {
    ...
    while s != 0 as *mut libc::c_void as *mut session {
        ...
        while c != 0 as *mut libc::c_void as *mut client {
            if !(0 != (*c).flags & 64i32) {
                if !((*c).flags & (8192i32 | 4194304i32) == 8192i32) {
                    if (*c).session == s {
                        if (*c).tty.sx < ssx { ssx = (*c).tty.sx }
                        (*c).flags &= !8388608i32;
                        if lines != 0i32 as libc::c_uint &&
                               lines.wrapping_add(2i32 as libc::c_uint) >
                                   (*c).tty.sy {
                            (*c).flags |= 8388608i32
                        }
                        ...
                        } else if (*c).tty.sy < ssy { ssy = (*c).tty.sy }
        ...
        if ssx ==
            ...
            (*s).flags &= !1i32;
            if lines != 0i32 as libc::c_uint && ssy == 0i32 as libc::c_uint {
                ssy = lines
            }
            if !((*s).sx == ssx && (*s).sy == ssy) {
                log_debug(b"session $%u size %u,%u (was %u,%u)\x00" as
                              *const u8 as *const libc::c_char, (*s).id, ssx,
                          ssy, (*s).sx, (*s).sy);
                (*s).sx = ssx;
                (*s).sy = ssy;
        ...
        if !((*w).active == 0 as *mut libc::c_void as *mut window_pane) {
            ...
            while s != 0 as *mut libc::c_void as *mut session {
                if !(0 != (*s).flags & 1i32) {
                    if 0 != flag {
                        has =
                            ((*(*s).curw).window == w) as libc::c_int as u_int
                    } else { has = session_has(s, w) as u_int }
                    if 0 != has {
                        if (*s).sx < ssx { ssx = (*s).sx }
                        if (*s).sy < ssy { ssy = (*s).sy }
                    }
                }
                s = sessions_RB_NEXT(s)
            }
            if !(ssx ==
                              ...
                              libc::c_uint).wrapping_mul(2u32).wrapping_add(1u32))
               {
                ...
                if limit >= 2i32 as libc::c_uint && ssx > limit {
                    ssx = limit;
                    forced |= 8192i32
                }
                limit =
                    options_get_number((*w).options,
                                       b"force-height\x00" as *const u8 as
                                           *const libc::c_char) as u_int;
                if limit >= 2i32 as libc::c_uint && ssy > limit {
                    ssy = limit;
                    forced |= 16384i32
                }
                if !((*w).sx == ssx && (*w).sy == ssy) {
                    log_debug(b"window @%u size %u,%u (was %u,%u)\x00" as
                                  *const u8 as *const libc::c_char, (*w).id,
                              ssx, ssy, (*w).sx, (*w).sy);
                    (*w).flags &= !(8192i32 | 16384i32);
                    (*w).flags |= forced;
                    is_zoomed = (*w).flags & 4096i32;
                    if 0 != is_zoomed { window_unzoom(w); }
                    layout_resize(w, ssx, ssy);
                    window_resize(w, ssx, ssy);
                    if 0 != is_zoomed && 0 != window_pane_visible((*w).active)
                       {
                        window_zoom((*w).active);
                    }
                    wp = (*w).active;
                    while 0 == window_pane_visible((*w).active) {
                        (*w).active =
                        ...
                        if (*w).active ==
                            ...
                            (*w).active =
                                *(*((*(&mut (*w).panes as
                                           *mut window_panes)).tqh_last as
                                        *mut window_panes)).tqh_last
                        }
                        if (*w).active == wp { break ; }
                    }
                    if (*w).active == (*w).last {
