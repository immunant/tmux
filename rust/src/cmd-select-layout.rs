...
unsafe extern "C" fn cmd_select_layout_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    next =
        ((*self_0).entry == &cmd_next_layout_entry as *const cmd_entry) as
            libc::c_int;
    if 0 != args_has(args, 110 as u_char) { next = 1i32 }
    previous =
        ((*self_0).entry == &cmd_previous_layout_entry as *const cmd_entry) as
            libc::c_int;
    if 0 != args_has(args, 112 as u_char) { previous = 1i32 }
    oldlayout = (*w).old_layout;
    (*w).old_layout = layout_dump((*w).layout_root);
    if 0 != next || 0 != previous {
        if 0 != next { layout_set_next(w); } else { layout_set_previous(w); }
    } else if 0 != args_has(args, 69 as u_char) {
        layout_spread_out(wp);
    } else {
        if 0 == args_has(args, 111 as u_char) {
            if (*args).argc == 0i32 {
                layout = (*w).lastlayout
            } else {
                layout = layout_set_lookup(*(*args).argv.offset(0isize))
            }
            if layout != 1i32.wrapping_neg() {
                layout_set_select(w, layout as u_int);
                current_block = 11053811894168143260;
            } else { current_block = 10879442775620481940; }
        } else { current_block = 10879442775620481940; }
        match current_block {
            11053811894168143260 => { }
            _ => {
                if (*args).argc != 0i32 {
                    layoutname = *(*args).argv.offset(0isize)
                } else if 0 != args_has(args, 111 as u_char) {
                    layoutname = oldlayout
                } else { layoutname = 0 as *const libc::c_char }
                if layoutname != 0 as *mut libc::c_void as *const libc::c_char
                   {
                    if layout_parse(w, layoutname) == 1i32.wrapping_neg() {
                        cmdq_error(item,
...
pub static mut cmd_next_layout_entry: cmd_entry =
    unsafe {
        cmd_entry{name:
                      b"next-layout\x00" as *const u8 as *const libc::c_char,
